...
const PlayerLoginChip = (props) => {
  return (
    <li>
      <Link onClick={() => {props.onAuthenticate(props.player)}} data-tip data-for={`player_tip_${props.player.id}`}
      to={`player/${props.player.id}`} className={`active`} style={{backgroundImage: `url('./${props.player.avatar_uri}')`}}></Link>
